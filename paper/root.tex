...
The mechanism is \replaced[id=Lg]{considered}{regarded} without the kinematic constraints by virtually cutting the loop-closing joints resulting in a tree structure \cite{KhalilBen1995} and removing the other constraints that are referred to as ``user constraints'' in \cite{SaminFis2013}.
...
The explicit form (\ref{equ:kinconstr_explicit}) on the contrary contains nested expressions with arctangent functions, since every successive substitution of the angles in the calculation of the closed loops \replaced[id=Lg]{results in}{brings} a new nested term of the arctangent function \replaced[id=Lg]{in}{into} the expression.
...
\end{equation}
%
to get the inverse dynamics joint torques\footnote{joint forces in the case of prismatic joints are also referred to as torques for the sake of readability} $\bm{\tau}^c_1$ of the constrained system in \replaced[id=Lg]{coordinates $\bm{q}_1$}{$\bm{q}_1$-coordinates}, which allows to obtain the dynamics equations in explicit form
%
\begin{equation}
...
\end{equation}
%
where \replaced[id=Lg]{$\bm{M}_1$, $\bm{c}_1$, $\bm{g}_1$, $\bm{\tau}_{1\mathrm{s}}$}{$\bm{M}$, $\bm{c}$, $\bm{g}$, $\bm{\tau}_{\mathrm{s}}$} denote the inertia matrix, and the vectors of centrifugal/Coriolis torques, gravity torques and spring torques, respectively.
...
Therefore \added[id=Lg]{it is not required to take} the angle of the cut joint \replaced[id=Lg]{into consideration}{does not have to be regarded}.
...
\end{align}
%
The projection matrix \added[id=Lg]{$\bm{W}$ from \cite{NakamuraGho1989,ParkChoPlo1999} is used in the form}
\vspace{-0.3cm}
%
\begin{equation}
...
\end{equation}
%
\deleted[id=Lg]{from \cite{NakamuraGho1989,ParkChoPlo1999} is used in the latter formulation of  (\ref{equ:kinconstr_impl_projmatrix})} \replaced[id=Lg]{in order to obtain}{to get} the joint torques $\bm{\tau}^c_1$ of the constrained system in minimal coordinates $\bm{q}_1$ as
%
\begin{equation}
...
%conclusion:
This paper presented a novel concept for a force assistance exoskeleton to support workers in carrying and using powertools\added[id=Lg]{ and its dynamics modeling}.
...
However, the presented approach does not \added[id=Lg]{completely} reach the \added[id=Lg]{computational} performance of standard solutions for the given mechanism.
...
%Future work:
\replaced[id=Lg]{In future works, the}{The} featured approach of implementing kinematic constraints will be applied to different serial-chain industrial robots with parallel mechanisms invoking kinematic constraints, i.\,e. hybrid robots. 
