...
for i = 1:n
  ...
  Mq = KAS5m7TE_inertiaJ_slag_vp1(q, ...
    pkin, m, rSges, Icges);
  T_M = 1/2 * qD' * Mq*qD;
...
for i = 1:n
  ...
  cq = KAS5m7TE_coriolisvecJ_fixb_slag_vp1(q, qD, ...
    pkin, m, rSges, Icges);

  % calculate with coriolis matrix
  Cq = KAS5m7TE_coriolismatJ_fixb_slag_vp2(q, qD, ...
    pkin, m, mrSges, Ifges);
  cq_mat = (Cq*qD);
...
for i = 1:n
  ...
  MqD = KAS5m7TE_inertiaDJ_slag_vp1(q, qD, ...
    pkin, m, rSges, Icges);
...
for i = 1:n
  ...
  g_base = G(i,:)';
  % Gravitation
  taug_par1 = KAS5m7TE_gravloadJ_floatb_twist_slag_vp1(q, g_base, ...
    pkin, m, rSges);
  taug_par2 = KAS5m7TE_gravloadJ_floatb_twist_slag_vp2(q, g_base, ...
    pkin, m, mrSges);
  ...
  % Coriolis
  tauc_par1 = KAS5m7TE_coriolisvecJ_fixb_slag_vp1(q, qD, ...
    pkin, m, rSges, Icges);
  tauc_par2 = KAS5m7TE_coriolisvecJ_fixb_slag_vp2(q, qD, ...
    pkin, m, mrSges, Ifges);
  ...
  % Inertia
  Mq_par1 = KAS5m7TE_inertiaJ_slag_vp1(q, ...
    pkin, m, rSges, Icges);
  Mq_par2 = KAS5m7TE_inertiaJ_slag_vp2(q, ...
    pkin, m, mrSges, Ifges);
  ...
  % Inverse Dynamics
  tau_par1 = KAS5m7TE_invdynJ_fixb_slag_vp1(q, qD, qDD, g_base, ...
    pkin, m, rSges, Icges);
  tau_par2 = KAS5m7TE_invdynJ_fixb_slag_vp2(q, qD, qDD, g_base, ...
    pkin, m, mrSges, Ifges);
  ...
  % Potentielle Energie
  U_par1 = KAS5m7TE_energypot_fixb_slag_vp1(q, g_base, ...
    pkin, m, rSges);
  U_par2 = KAS5m7TE_energypot_fixb_slag_vp2(q, g_base, ...
    pkin, m, mrSges);
  if any(abs(U_par1-U_par2) > thrfac*1e6*eps(U_par1))
    error('Potential energies do not match between par1/par2.');
  end
  % Kinetische Energie
  T_par1 = KAS5m7TE_energykin_fixb_slag_vp1(q, qD, ...
    pkin, m, rSges, Icges);
  T_par2 = KAS5m7TE_energykin_fixb_slag_vp2(q, qD, ...
    pkin, m, mrSges, Ifges);
...
for i = 1:n
  ...
  g_base = G(i,:)';
  % Gravitation
  taug_id = KAS5m7TE_invdynJ_fixb_slag_vp1(q, zeros(NQJ,1), zeros(NQJ,1), g_base, ...
    pkin, m, rSges, Icges);
  taug_sp = KAS5m7TE_gravloadJ_floatb_twist_slag_vp1(q, g_base, ...
    pkin, m, rSges);
  ...
  % Coriolis
  tauc_id = KAS5m7TE_invdynJ_fixb_slag_vp1(q, qD, zeros(NQJ,1), zeros(3,1), ...
    pkin, m, rSges, Icges);
  tauc_sp = KAS5m7TE_coriolisvecJ_fixb_slag_vp1(q, qD, ...
    pkin, m, rSges, Icges);
  ...
  % Inertia
  taua_id = KAS5m7TE_invdynJ_fixb_slag_vp1(q, zeros(NQJ,1), qDD, zeros(3,1), ...
    pkin, m, rSges, Icges);
  Mq = KAS5m7TE_inertiaJ_slag_vp1(q, ...
    pkin, m, rSges, Icges);
  taua_sp = Mq*qDD;
...
for ii = 1:n
  ...
  odeM = @(x) KAS5m7TE_inertiaJ_slag_vp1(x(1:NQJ), ...
    pkin, m, rSges, Icges);
  odeInvDyn = @(x) KAS5m7TE_invdynJ_fixb_slag_vp1(x(1:NQJ), ...
  ...
  for i = 1:length(t)
    ...
    E(i,1) = KAS5m7TE_energykin_fixb_slag_vp1(q, qD, pkin, m, rSges, Icges);
    E(i,2) = KAS5m7TE_energypot_fixb_slag_vp1(q, g_base, pkin, m, rSges);
    tau_Acc(i,:) = KAS5m7TE_invdynJ_fixb_slag_vp1(q, qD, zeros(NQJ,1), g_base, pkin, m, rSges, Icges)';
  end
