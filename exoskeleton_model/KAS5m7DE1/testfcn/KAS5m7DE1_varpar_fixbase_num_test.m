...
% * numeric robot-specific functions from template of this toolbox (KAS5m7DE1__...nnew_...)
% with fixed base model

% Quelle: HybrDyn-Toolbox
% Datum: 2020-05-23 16:48
...
%% Parameter
TSS = KAS5m7DE1_varpar_testfunctions_parameter();
for f = fields(TSS)'
...
MPV = KAS5m7DE1_convert_par2_MPV_fixb(pkin, m, mrSges, Ifges);
...
for i = 1:n
  ...
  [~, T_stack] = KAS5m7DE1_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);

  % Gravitation
  taug_sym = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp1(qJ, g_base, ...
    pkin, m, rSges);
  ...
  taug_num3 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ, zeros(NJ,1), zeros(NJ,1), zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], ...
    pkin, m, rSges, Icges);
  ...
  taug_num4 = KAS5m7DE1_gravload_floatb_eulxyz_nnew_vp1(qJ, zeros(3,1), g_base, ...
    pkin, m, rSges);
  ...
  taug_num5 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp2(qJ, zeros(NJ,1), zeros(NJ,1), zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], ...
    pkin, m, mrSges, Ifges);
  ...
  % Coriolis
  tauc_sym = KAS5m7DE1_coriolisvecJ_fixb_slag_vp1(qJ, qJD, ...
    pkin, m, rSges, Icges);
  ...
  tauc_num2 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ, qJD, zeros(NJ,1), zeros(3,1), zeros(6,1), zeros(6,1), ...
    pkin, m, rSges, Icges);
  ...
  tauc_num4 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp2(qJ, qJD, zeros(NJ,1), zeros(3,1), zeros(6,1), zeros(6,1), ...
    pkin, m, mrSges, Ifges);
  ...
  % Massenmatrix
  Mq_sym = KAS5m7DE1_inertiaJ_slag_vp1(qJ, ...
    pkin, m, rSges, Icges);
  Mq_num1 = NaN(NJ,NJ);
  Mq_num2 = NaN(NJ,NJ);
  Mq_num3 = NaN(NJ,NJ);
  ...
  for jj = 1:NJ
    Mq_num1(:,jj) = [zeros(NJ,6), eye(NJ)]*robot_tree_invdyn_floatb_eulxyz_mdh_nnew_vp1  (qJ, zeros(NJ,1), INJ(:,jj), zeros(3,1), zeros(6,1), zeros(6,1) , ...
      alpha, a, d, theta, q_offset, b, beta, v, sigma, m, rSges, Icges);
    Mq_num2(:,jj) = [zeros(NJ,6), eye(NJ)]*KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ, zeros(NJ,1), INJ(:,jj), zeros(3,1), zeros(6,1), zeros(6,1) , ...
      pkin, m, rSges, Icges);
    Mq_num3(:,jj) = [zeros(NJ,6), eye(NJ)]*KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp2(qJ, zeros(NJ,1), INJ(:,jj), zeros(3,1), zeros(6,1), zeros(6,1) , ...
      pkin, m, mrSges, Ifges);
  end
  ...
  Mq_num4 = KAS5m7DE1_inertiaJ_nCRB_vp1(qJ, ...
    pkin, m, rSges, Icges);
  ...
  [~, ~, ~, f_i_i_ges1, n_i_i_ges1] = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ, qJD, qJDD, zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], ...
      pkin, m, rSges, Icges);
  [~, ~, ~, f_i_i_ges2, n_i_i_ges2] = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp2(qJ, qJD, qJDD, zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], ...
      pkin, m, mrSges, Ifges);
  ...
  tauJ_sym = KAS5m7DE1_invdynJ_fixb_snew_vp2(qJ, qJD,  qJDD, g_base, ...
    pkin, m, mrSges, Ifges);
  tauB_sym = KAS5m7DE1_invdynB_fixb_snew_vp2(qJ, qJD,  qJDD, g_base, ...
    pkin, m, mrSges, Ifges);
  ...
  [tau_num2,~,~,f_neweul_num,m_neweul_num] = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ, qJD, qJDD, zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)] , ...
    pkin, m, rSges, Icges);
  ...
  f_neweul_sym = KAS5m7DE1_invdynf_fixb_snew_vp2(qJ, qJD, qJDD, g_base, ...
    pkin, m, mrSges, Ifges);
  m_neweul_sym = KAS5m7DE1_invdynm_fixb_snew_vp2(qJ, qJD, qJDD, g_base, ...
    pkin, m, mrSges, Ifges);
...
for i = 1:n
  ...
  [~, Tc_stack] = KAS5m7DE1_fkine_fixb_rotmat_mdh_sym_varpar(qJ, pkin);
  ...
  for j = uint8(0:NL-1)
    ...
    J_rob = KAS5m7DE1_jacobig_mdh_num(qJ, j, r_i_i_C, pkin);
    test_J = J_gen - J_rob;
  ...
  matlabfcn2mex({'KAS5m7DE1_jacobig_cutforce_mdh_num'});
  for j = uint8(0:NL-1)
    r_i_i_C = rand(3,1);
    Jg_C1 = robot_tree_jacobig_cutforce_m(Tc_stack, v, j, r_i_i_C);
    Jg_C2 = KAS5m7DE1_jacobig_cutforce_mdh_num(qJ, j, r_i_i_C, pkin);
    test_JC = Jg_C1 - Jg_C2;
    if max(abs(test_JC(:))) > 1e-10
      error('Schnittkraft-Jacobi-Matrix unterscheidet sich zwischen zwei Implementierungen');
    end
    Jg_C2m = KAS5m7DE1_jacobig_cutforce_mdh_num_mex(qJ, j, r_i_i_C, pkin);
  ...
  for j = uint8(0:NL-1)
    ...
    f_neweul_sym = KAS5m7DE1_invdynf_fixb_snew_vp2(qJ, 0*qJ, 0*qJ, g_base, ...
      pkin, m_test, mrSges_test, 0*Ifges);
    m_neweul_sym = KAS5m7DE1_invdynm_fixb_snew_vp2(qJ, 0*qJ, 0*qJ, g_base, ...
      pkin, m_test, mrSges_test, 0*Ifges);
...
matlabfcn2mex({'KAS5m7DE1_gravloadJ_floatb_twist_slag_vp1', ...
  'KAS5m7DE1_gravloadJ_floatb_twist_slag_vp2', ...
  'KAS5m7DE1_gravloadJ_regmin_slag_vp', ...
  'KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1', ...
  'KAS5m7DE1_gravload_floatb_eulxyz_nnew_vp1'});
n = 2000;
% Berechne Rechenzeiten. Zeile 1: Normale Funktionen. Zeile 2: Kompilierte Fkt (mex)
t_ber_g = NaN(2,7);
tic;
for i = 1:n
  taug_sym = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp1(qJ+i*1e-3, g_base, ...
    pkin, m, rSges);
end
t_ber_g(1,1) = toc/n;
tic;
for i = 1:n
  taug_sym = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp1_mex(qJ+i*1e-3, g_base, ...
    pkin, m, rSges);
end
t_ber_g(2,1) = toc/n;
tic;
for i = 1:n
  taug_sym = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp2(qJ+i*1e-3, g_base, ...
    pkin, m, mrSges);
end
t_ber_g(1,2) = toc/n;
tic;
for i = 1:n
  taug_sym = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp2_mex(qJ+i*1e-3, g_base, ...
    pkin, m, mrSges);
end
t_ber_g(2,2) = toc/n;
tic;
for i = 1:n
  t_ber_g(2,2) = toc/n;
    taug_reg = KAS5m7DE1_gravloadJ_regmin_slag_vp(qJ+i*1e-3, g_base, pkin);
...
for i = 1:n
  t_ber_g(2,2) = toc/n;
    taug_reg = KAS5m7DE1_gravloadJ_regmin_slag_vp_mex(qJ+i*1e-3, g_base, pkin);
    taug_mpv = taug_reg*MPV;
end
t_ber_g(2,3) = toc/n;
...
tic;
for i = 1:n
  taug_num3 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ+i*1e-3, zeros(NJ,1), zeros(NJ,1), zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], pkin, m, rSges, Icges);
end
t_ber_g(1,6) = toc/n;
tic;
for i = 1:n
  taug_num3 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1_mex(qJ+i*1e-3, zeros(NJ,1), zeros(NJ,1), zeros(3,1), zeros(6,1), [-g_base;zeros(3,1)], pkin, m, rSges, Icges);
end
t_ber_g(2,6) = toc/n;
tic;
for i = 1:n
  taug_num4 = KAS5m7DE1_gravload_floatb_eulxyz_nnew_vp1(qJ+i*1e-3, zeros(3,1), g_base, ...
    pkin, m, rSges);
end
t_ber_g(1,7) = toc/n;
tic;
for i = 1:n
  taug_num4 = KAS5m7DE1_gravload_floatb_eulxyz_nnew_vp1_mex(qJ+i*1e-3, zeros(3,1), g_base, ...
    pkin, m, rSges);
end
t_ber_g(2,7) = toc/n;
...
% Symbolisch, numerisch, mit inverser Dynamik und mit Spezialfunktione,
% kompiliert (mex) und normaler Aufruf
matlabfcn2mex({'KAS5m7DE1_coriolisvecJ_fixb_slag_vp1', ...
  'KAS5m7DE1_coriolisvecJ_fixb_slag_vp2', ...
  'KAS5m7DE1_coriolisvecJ_fixb_regmin_slag_vp', ...
  'KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1'});
n = 2000;
...
tic;
for i = 1:n
  tauc_sym = KAS5m7DE1_coriolisvecJ_fixb_slag_vp1(qJ+i*1e-3, qJD,...
    pkin, m, rSges, Icges);
end
t_ber_c(1,1) = toc/n;
tic;
for i = 1:n
  tauc_sym = KAS5m7DE1_coriolisvecJ_fixb_slag_vp1_mex(qJ+i*1e-3, qJD,...
    pkin, m, rSges, Icges);
end
t_ber_c(2,1) = toc/n;
tic;
for i = 1:n
  tauc_sym = KAS5m7DE1_coriolisvecJ_fixb_slag_vp2(qJ+i*1e-3, qJD,...
    pkin, m, mrSges, Ifges);
end
t_ber_c(1,2) = toc/n;
tic;
for i = 1:n
  tauc_sym = KAS5m7DE1_coriolisvecJ_fixb_slag_vp2_mex(qJ+i*1e-3, qJD,...
    pkin, m, mrSges, Ifges);
end
t_ber_c(2,2) = toc/n;
tic;
for i = 1:n
  tauc_reg = KAS5m7DE1_coriolisvecJ_fixb_regmin_slag_vp(qJ+i*1e-3, qJD, ...
    pkin);
...
tic;
for i = 1:n
  tauc_reg = KAS5m7DE1_coriolisvecJ_fixb_regmin_slag_vp_mex(qJ+i*1e-3, qJD, ...
    pkin);
  tauc_mpv = tauc_reg*MPV;
end
t_ber_c(2,3) = toc/n;
...
tic;
for i = 1:n
  tauc_num2 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ+i*1e-3, qJD, zeros(NJ,1), zeros(3,1), zeros(6,1), zeros(6,1), ...
    pkin, m, rSges, Icges);
end
t_ber_c(1,5) = toc/n;
tic;
for i = 1:n
  tauc_num3 = KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1_mex(qJ+i*1e-3, qJD, zeros(NJ,1), zeros(3,1), zeros(6,1), zeros(6,1), ...
    pkin, m, rSges, Icges);
end
t_ber_c(2,5) = toc/n;
...
% Symbolisch, numerisch, mit inverser Dynamik und mit Spezialfunktione,
% kompiliert (mex) und normaler Aufruf
matlabfcn2mex({'KAS5m7DE1_inertia_floatb_eulxyz_slag_vp1', ...
  'KAS5m7DE1_inertia_floatb_eulxyz_slag_vp2', ...
  'KAS5m7DE1_inertiaJ_slag_vp1', ...
  'KAS5m7DE1_inertiaJ_slag_vp2', ...
  'KAS5m7DE1_inertiaJ_regmin_slag_vp', ...
  'KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1', ...
  'KAS5m7DE1_inertiaJ_nCRB_vp1'});
n = 200;
t_ber_M = NaN(2,4);
tic;
for i = 1:n
  Mq_sym = KAS5m7DE1_inertiaJ_slag_vp1(qJ+1e-3*i, ...
    pkin, m, rSges, Icges);
end
t_ber_M(1,1) = toc/n;
tic;
for i = 1:n
  Mq_sym = KAS5m7DE1_inertiaJ_slag_vp1_mex(qJ+1e-3*i, ...
    pkin, m, rSges, Icges);
end
t_ber_M(2,1) = toc/n;
tic;
for i = 1:n
  Mq_sym = KAS5m7DE1_inertiaJ_slag_vp2(qJ+1e-3*i, ...
    pkin, m, mrSges, Ifges);
end
t_ber_M(1,2) = toc/n;
tic;
for i = 1:n
  Mq_sym = KAS5m7DE1_inertiaJ_slag_vp2_mex(qJ+1e-3*i, ...
    pkin, m, mrSges, Ifges);
end
t_ber_M(2,2) = toc/n;
tic;
for i = 1:n
  Mq_reg = KAS5m7DE1_inertiaJ_regmin_slag_vp(qJ, ...
    pkin);
...
tic;
for i = 1:n
  Mq_reg = KAS5m7DE1_inertiaJ_regmin_slag_vp_mex(qJ, ...
    pkin);
  Mq_mpv_vec = Mq_reg*MPV;
  Mq_mpv = vec2symmat(Mq_mpv_vec);
end
...
for i = 1:50
  ...
  INJ = eye(NJ);
  for jj = 1:NJ
    Mq_num2(:,jj) = [zeros(NJ,6), eye(NJ)]*KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1(qJ+1e-3*i, zeros(NJ,1), INJ(:,jj), zeros(3,1), zeros(6,1), zeros(6,1) , ...
      pkin, m, rSges, Icges);
  end
end
t_ber_M(1,5) = toc/50;
...
for i = 1:n
  ...
  INJ = eye(NJ);
  for jj = 1:NJ
    Mq_num2(:,jj) = [zeros(NJ,6), eye(NJ)]*KAS5m7DE1_invdyn_floatb_eulxyz_nnew_vp1_mex(qJ+1e-3*i, zeros(NJ,1), INJ(:,jj), zeros(3,1), zeros(6,1), zeros(6,1) , ...
      pkin, m, rSges, Icges);
  end
end
t_ber_M(2,5) = toc/n;
tic
for i = 1:50
  Mq_num3 = KAS5m7DE1_inertiaJ_nCRB_vp1(qJ, ...
    pkin, m, rSges, Icges);
end
t_ber_M(1,6) = toc/50;
...
tic
for i = 1:n
  Mq_num4 = KAS5m7DE1_inertiaJ_nCRB_vp1_mex(qJ, ...
    pkin, m, rSges, Icges);
end
t_ber_M(2,6) = toc/n;
