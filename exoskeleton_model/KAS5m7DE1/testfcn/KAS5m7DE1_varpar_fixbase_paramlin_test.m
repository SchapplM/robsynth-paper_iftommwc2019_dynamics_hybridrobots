...
for i = 1:n
  ...
  t_reg2 = KAS5m7DE1_energykin_fixb_reg2_slag_vp(q, qD, ...
    pkin);
...
for i = 1:n
  ...
  U_func = KAS5m7DE1_energypot_fixb_slag_vp1(q, g_base, ...
    pkin, m, rSges);
  U_func0 = KAS5m7DE1_energypot_fixb_slag_vp1(Q(1,:)', g_base, ...
    pkin, m, rSges);
  % calculate potential energy with optimized function
  if ~KINCONSTR && ~TREE
    % calculate potential energy with minimal regressor
    u_regmin = KAS5m7DE1_energypot_fixb_regmin_slag_vp(q, g_base, ...
      pkin);
    u_regmin0 = KAS5m7DE1_energypot_fixb_regmin_slag_vp(Q(1,:)', g_base, ...
      pkin);
    U_mpv = u_regmin*MPV;
  ...
  u_reg2 = KAS5m7DE1_energypot_fixb_reg2_slag_vp(q, g_base, ...
    pkin);
  u_reg20 = KAS5m7DE1_energypot_fixb_reg2_slag_vp(Q(1,:)', g_base, ...
    pkin);
...
for i = 1:n
  q = Q(i,:)';
  g_base = G(i,:)';
  taug_func = KAS5m7DE1_gravloadJ_floatb_twist_slag_vp1(q, g_base, ...
    pkin, m, rSges);
  % calculate gravitational load with optimized function
  if ~KINCONSTR && ~TREE
    % calculate gravitational load with base parameter regressor
    taug_regmin = KAS5m7DE1_gravloadJ_regmin_slag_vp(q, g_base, ...
      pkin);
  ...
  taug_reg2 = KAS5m7DE1_gravloadJ_reg2_slag_vp(q, g_base, ...
    pkin);
...
for i = 1:n
  q = Q(i,:)';
  Mq_func = KAS5m7DE1_inertiaJ_slag_vp1(q, pkin, m, rSges, Icges);
  % calculate inertia with optimized function
  if ~KINCONSTR && ~TREE
    % calculate inertia with base parameter regressor
    Mq_regmin = KAS5m7DE1_inertiaJ_regmin_slag_vp(q, pkin);
  ...
  Mq_reg2 = KAS5m7DE1_inertiaJ_reg2_slag_vp(q, pkin);
...
for i = 1:n
  ...
  MqD_reg2 = KAS5m7DE1_inertiaDJ_reg2_slag_vp(q, qD, pkin);
...
if ~KINCONSTR
  for i = 1:n
    ...
    qDD = QDD(i,:)';
    g_base = G(i,:)';
    tauJ_neweul_func = KAS5m7DE1_invdynJ_fixb_snew_vp2(q, qD, qDD, g_base, ...
      pkin, m, mrSges, Ifges);
    % calculate inverse dynamics with inertial parameter regressor
    tauJ_neweul_reg2 = KAS5m7DE1_invdynJ_fixb_reg2_snew_vp(q, qD, qDD, g_base, ...
      pkin);
  ...
  for i = 1:n
    ...
    qDD = QDD(i,:)';
    g_base = G(i,:)';
    tauB_neweul_func = KAS5m7DE1_invdynB_fixb_snew_vp2(q, qD, qDD, g_base, ...
      pkin, m, mrSges, Ifges);
    % calculate base forces with inertial parameter regressor
    tauB_neweul_reg2 = KAS5m7DE1_invdynB_fixb_reg2_snew_vp(q, qD, qDD, g_base, ...
      pkin);
  ...
  for i = 1:n
    ...
    qDD = QDD(i,:)';
    g_base = G(i,:)';
    f_neweul_func = KAS5m7DE1_invdynf_fixb_snew_vp2(q, qD, qDD, g_base, ...
      pkin, m, mrSges, Ifges);
    % calculate cutting forces with inertial parameter regressor
    f_neweul_reg2 = KAS5m7DE1_invdynf_fixb_reg2_snew_vp(q, qD, qDD, g_base, ...
      pkin);
  ...
  for i = 1:n
    ...
    qDD = QDD(i,:)';
    g_base = G(i,:)';
    m_neweul_func = KAS5m7DE1_invdynm_fixb_snew_vp2(q, qD, qDD, g_base, ...
      pkin, m, mrSges, Ifges);
    % calculate cutting torques with inertial parameter regressor
    m_neweul_reg2 = KAS5m7DE1_invdynm_fixb_reg2_snew_vp(q, qD, qDD, g_base, ...
      pkin);
...
if ~KINCONSTR && ~TREE
  for i = 1:n
    ...
    taug_regmin = KAS5m7DE1_gravloadJ_regmin_slag_vp(q, g_base, pkin);
    taug_mpv = taug_regmin*MPV;
    taug_func = KAS5m7DE1_gravloadJ_floatb_twist_mdp_slag_vp(q, g_base, pkin, MPV);
    ...
    tauc_regmin = KAS5m7DE1_coriolisvecJ_fixb_regmin_slag_vp(q, qD, pkin);
    tauc_mpv = tauc_regmin*MPV;
    tauc_func = KAS5m7DE1_coriolisvecJ_fixb_mdp_slag_vp(q, qD, pkin, MPV);
    ...
    Mq_mpv_vec = Mq_regmin*MPV;
    Mq_mpv = vec2symmat(Mq_mpv_vec);
    Mq_func = KAS5m7DE1_inertiaJ_mdp_slag_vp(q, pkin, MPV);
    if any( abs(Mq_func(:)-Mq_mpv(:)) > thrfac*1e6*eps(max(abs(Mq_func(:)))) )
      error('Inertia matrix from base parameter regressor does not match with pre-multiplied calculation.');
    end

    tau_regmin = KAS5m7DE1_invdynJ_fixb_regmin_slag_vp(q, qD, qDD, g_base, ...
      pkin);
    tau_mpv = tau_regmin*MPV;
    tau_func = KAS5m7DE1_invdynJ_fixb_mdp_slag_vp(q, qD, qDD, g_base, pkin, MPV);
...
if ~KINCONSTR && ~TREE
  ...
  RV_Traj = KAS5m7DE1_invdynJ_fixb_regmin_slag_vp_traj(Q, QD, QDD, g_base, pkin);
  % Damit Zeitreihe der Momente berechnen
  TAU_vr = KAS5m7DE1_invdynJ_fixb_mdp_slag_vr_traj(RV_Traj, MPV);
  % Zeitreihe der Momente über direkten Funktionsaufruf
  TAU_vp = KAS5m7DE1_invdynJ_fixb_mdp_slag_vp_traj(Q, QD, QDD, g_base, pkin, MPV);
  ...
  matlabfcn2mex({ 'KAS5m7DE1_invdynJ_fixb_regmin_slag_vp_traj', ...
                  'KAS5m7DE1_invdynJ_fixb_mdp_slag_vr_traj', ...
                  'KAS5m7DE1_invdynJ_fixb_mdp_slag_vp_traj'});
end
...
% [Sousa2014] Gl. (38)
[K, K_d, P_b, P_d] = KAS5m7DE1_PV2_MPV_transformations_fixb(pkin);
...
for i = 1:n
  ...
  g_base = G(i,:)';
  % [Gautier1990] Gl. (7)
  tau_reg2 = KAS5m7DE1_invdynJ_fixb_reg2_slag_vp(q, qD, qDD, g_base, ...
    pkin);
  ...
  u_reg2 = KAS5m7DE1_energypot_fixb_reg2_slag_vp(q, g_base, pkin);
  u_reg20 = KAS5m7DE1_energypot_fixb_reg2_slag_vp(Q(1,:)', g_base, pkin);
  t_reg2 = KAS5m7DE1_energykin_fixb_reg2_slag_vp(q, qD, pkin);
  W_g_energy(i,:) = t_reg2 + u_reg2 - u_reg20;
end
% Struktur mit Eingabedaten für Algorithmus generieren
[Ksym, K_dsym, P1sym, P2sym] = KAS5m7DE1_PV2_MPV_transformations_fixb(pkin);
