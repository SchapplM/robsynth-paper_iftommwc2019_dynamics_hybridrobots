...
function jv = KAS5m5_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(5,1),zeros(30,1)}
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'KAS5m5_kinconstr_expl_mdh_num_varpar: qJ has to be [5x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [30 1]), ...
  'KAS5m5_kinconstr_expl_mdh_num_varpar: pkin has to be [30x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = KAS5m5_pkin2mdhparam(pkin);
[~, sigma_mdh] = KAS5m5_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = KAS5m5_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
