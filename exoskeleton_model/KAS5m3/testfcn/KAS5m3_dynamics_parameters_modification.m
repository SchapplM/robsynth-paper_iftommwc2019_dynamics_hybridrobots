...
function [m,rSges,Icges] = KAS5m3_dynamics_parameters_modification(pkin,m,rSges,Icges)

%% Coder Information
%#codegen
%$cgargs {zeros(30,1),zeros(14,1),zeros(14,3),zeros(14,6)}
assert(isreal(pkin) && all(size(pkin) == [30 1]), ...
  'KAS5m3_dynamics_parameters_modification: pkin has to be [30x1] (double)');
assert(isreal(m) && all(size(m) == [14 1]), ...
  'KAS5m3_dynamics_parameters_modification: m has to be [14x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [14,3]), ...
  'KAS5m3_dynamics_parameters_modification: rSges has to be [14x3] (double)');
assert(isreal(Icges) && all(size(Icges) == [14 6]), ...
  'KAS5m3_dynamics_parameters_modification: Icges has to be [14x6] (double)'); 
