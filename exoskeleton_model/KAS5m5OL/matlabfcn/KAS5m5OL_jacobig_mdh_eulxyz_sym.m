...
%     and control
%
% Siehe auch: KAS5m5OL_jacobig_mdh_eulxyz_num.m
...
function Jg_C = KAS5m5OL_jacobig_mdh_eulxyz_sym(phi_base, qJ, link_index, r_i_i_C, pkin)
%% Init
%#codegen
%$cgargs {zeros(3,1),zeros(13,1),uint8(zeros(1,1)),zeros(3,1),zeros(12,1)}
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'KAS5m5OL_jacobig_mdh_eulxyz_sym: Base Euler-XYZ angles have to be [3x1] (double)');
assert(isreal(qJ) && all(size(qJ) == [13 1]), ...
  'KAS5m5OL_jacobig_mdh_eulxyz_sym: Joint angles qJ have to be [13x1] (double)');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'KAS5m5OL_jacobig_mdh_eulxyz_sym: Position vector r_i_i_C has to be [3x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
  'KAS5m5OL_jacobig_mdh_eulxyz_sym: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [12 1]), ...
  'KAS5m5OL_jacobig_mdh_eulxyz_sym: Kinematic parameters pkin have to be [12x1] (double)');
if link_index > 14-1
...
Jg_JC_B = KAS5m5OL_jacobig_sym_varpar(qJ, link_index, r_i_i_C, pkin);
