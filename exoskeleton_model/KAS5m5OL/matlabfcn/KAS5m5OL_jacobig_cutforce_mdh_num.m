...
function Jg_C = KAS5m5OL_jacobig_cutforce_mdh_num(qJ, link_index, r_i_i_C, pkin)

%% Init
%#codegen
%$cgargs {zeros(13,1),uint8(zeros(1,1)),zeros(3,1),zeros(12,1)}
assert(isreal(qJ) && all(size(qJ) == [13 1]), ...
  'KAS5m5OL_jacobig_cutforce_mdh_num: Joint angles qJ have to be [13x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
  'robot_tree_jacobig_m: link_index has to be [1x1] uint8');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'KAS5m5OL_jacobig_cutforce_mdh_num: Position vector r_i_i_C has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [12 1]), ...
  'KAS5m5OL_jacobig_cutforce_mdh_num: Kinematic parameters pkin have to be [12x1] (double)');

T_c_mdh = KAS5m5OL_fkine_fixb_rotmat_mdh_sym_varpar(qJ, pkin);
[v,~,~,NL] = KAS5m5OL_structural_kinematic_parameters();
